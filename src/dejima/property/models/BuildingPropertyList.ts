...
import {
  ...
  BuildingPropertyPreviewFromJSON,
  BuildingPropertyPreviewFromJSONTyped,
  BuildingPropertyPreviewToJSON,
} from './';

/**
 * 賃貸物件概要一覧（建物・棟別表示）
 * @export
 * @interface BuildingPropertyList
 */
export interface BuildingPropertyList {
  /**
   * 件数
   * @type {number}
   * @memberof BuildingPropertyList
   */
  totalCounts: number;
  /**
   * 検索の開始インデックス
  ...
  /**
   * 棟表示物件概要
   * @type {Array<BuildingPropertyPreview>}
   * @memberof BuildingPropertyList
   */
  results: Array<BuildingPropertyPreview>;
}

export function BuildingPropertyListFromJSON(json: any): BuildingPropertyList {
  return BuildingPropertyListFromJSONTyped(json, false);
}

export function BuildingPropertyListFromJSONTyped(
  json: any,
...
export function BuildingPropertyListToJSON(
  value?: BuildingPropertyList | null
): any {
  if (value === undefined) {
    return undefined;
  }
  ...
  return {
    total_counts: value.totalCounts,
    start_index: value.startIndex,
    items_per_page: value.itemsPerPage,
    results: (value.results as Array<any>).map(BuildingPropertyPreviewToJSON),
  };
}
