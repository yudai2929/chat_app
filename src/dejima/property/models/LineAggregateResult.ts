...
import {
  LineAggregateElem,
  LineAggregateElemFromJSON,
  LineAggregateElemFromJSONTyped,
  LineAggregateElemToJSON,
} from './';

/**
 * 駅・沿線別集計結果
 * @export
 * @interface LineAggregateResult
 */
export interface LineAggregateResult {
  /**
   * 集計のレベル
   * @type {string}
   * @memberof LineAggregateResult
   */
  level: string;
  /**
   * 集計結果
   * @type {Array<LineAggregateElem>}
   * @memberof LineAggregateResult
   */
  results: Array<LineAggregateElem>;
}

export function LineAggregateResultFromJSON(json: any): LineAggregateResult {
  return LineAggregateResultFromJSONTyped(json, false);
}

export function LineAggregateResultFromJSONTyped(
  json: any,
  ...
  return {
    level: json['level'],
    results: (json['results'] as Array<any>).map(LineAggregateElemFromJSON),
  };
}

export function LineAggregateResultToJSON(
  value?: LineAggregateResult | null
): any {
  if (value === undefined) {
    return undefined;
  }
  ...
  return {
    level: value.level,
    results: (value.results as Array<any>).map(LineAggregateElemToJSON),
  };
}
