 ...
 * 接道状況
 * @export
 * @interface FacingRoad
 */
export interface FacingRoad {
  /**
   *
   * @type {string}
   * @memberof FacingRoad
   */
  readonly roadKind?: string;
  /**
   * 1: 公道<br/>2: 私道
   * @type {number}
   * @memberof FacingRoad
   ...
   * @memberof FacingRoad
   */
  roadPositionFlag?: boolean;
}

export function FacingRoadFromJSON(json: any): FacingRoad {
  return FacingRoadFromJSONTyped(json, false);
}

export function FacingRoadFromJSONTyped(
  json: any,
  ...
  return {
    roadKind: !exists(json, 'road_kind') ? undefined : json['road_kind'],
    roadKindCode: !exists(json, 'road_kind_code')
      ? undefined
      : json['road_kind_code'],
    roadDirection: !exists(json, 'road_direction')
      ? undefined
      : json['road_direction'],
    roadDirectionCode: !exists(json, 'road_direction_code')
      ? undefined
      : json['road_direction_code'],
    roadWidth: !exists(json, 'road_width') ? undefined : json['road_width'],
    roadFacingLength: !exists(json, 'road_facing_length')
      ? undefined
      : json['road_facing_length'],
    isPaved: !exists(json, 'is_paved') ? undefined : json['is_paved'],
    roadPositionFlag: !exists(json, 'road_position_flag')
      ? undefined
      : json['road_position_flag'],
  };
}

export function FacingRoadToJSON(value?: FacingRoad | null): any {
  if (value === undefined) {
    return undefined;
  }
  if (value === null) {
  ...
  return {
    road_kind_code: value.roadKindCode,
    road_direction_code: value.roadDirectionCode,
    road_width: value.roadWidth,
    road_facing_length: value.roadFacingLength,
    is_paved: value.isPaved,
    road_position_flag: value.roadPositionFlag,
  };
}
